...
def _aggregate_bold(func_ls):
    ...
    for f in func_ls[:]:
        ...
        if suffix == f[-len(suffix):]:
            if func_runs.get(series_id) is None: func_runs[series_id] = _set_run()
...
def _set_run():
    ...
    return {'json_bold':None,'mag_bold':None,'events_tsv':None,'json_sbref':None,'mag_sbref':None,'physio':None}
