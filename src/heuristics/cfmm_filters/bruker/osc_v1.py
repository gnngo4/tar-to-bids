    ...
    def get_info(self):
        ...
        for s in self.seqinfo:
            ...
            if 'task-visualblock_res-A' in description:
                template = create_key(f'sub-{{subject}}/{{session}}/func/sub-{{subject}}_{{session}}_task-TEST01VisualBlock_dir-AP_run-{{item:02d}}_bold')
            elif 'task-visualosc_res-A' in description:
                template = create_key(f'sub-{{subject}}/{{session}}/func/sub-{{subject}}_{{session}}_task-TEST01VisualFT_dir-AP_run-{{item:02d}}_bold')
            elif 'task-visualblock_res-B' in description:
                template = create_key(f'sub-{{subject}}/{{session}}/func/sub-{{subject}}_{{session}}_task-TEST02VisualBlock_dir-AP_run-{{item:02d}}_bold')    
            elif 'task-visualosc_res-B' in description:
                template = create_key(f'sub-{{subject}}/{{session}}/func/sub-{{subject}}_{{session}}_task-TEST02VisualFT_dir-AP_run-{{item:02d}}_bold')    
            else:
                continue
