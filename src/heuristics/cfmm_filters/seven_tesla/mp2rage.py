...
class mp2rage:
    ...
    def get_info(self):
        ...
        uni = create_key(
            "sub-{subject}/{session}/anat/sub-{subject}_{session}_acq-UNI_run-{item:02d}_MP2RAGE"
        )
        t1map = create_key(
            "sub-{subject}/{session}/anat/sub-{subject}_{session}_acq-MP2RAGE_run-{item:02d}_T1map"
        )
        inv1 = create_key(
            "sub-{subject}/{session}/anat/sub-{subject}_{session}_inv-1_run-{item:02d}_part-mag_MP2RAGE"
        )
        inv2 = create_key(
            "sub-{subject}/{session}/anat/sub-{subject}_{session}_inv-2_run-{item:02d}_part-mag_MP2RAGE"
        )
